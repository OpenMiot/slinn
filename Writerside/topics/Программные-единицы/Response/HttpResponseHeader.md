...
Объект класса является сборщиком заголовка HTTP-ответа из объектного представления

Наследуется от `slinn.HttpResponseChunk`

```Python
class slinn.HttpResponseHeader (
...
```
1. `data` - HTTP-заголовки ответа;
2. `status` - HTTP-код ответа;
3. `content_type` - `Content-Type` заголовок;
4. `use_gzip` - использовать сжатие gzip (устанавливает HTTP-заголовок `Content-Encoding: gzip`).
...
<table width="100%">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><code>[('Content-Type', 'text/event-stream; charset=utf-8'), ('Server', slinn.version), ('Cache-Control', 'no-cache'), ('Connection', 'Keep-Alive')]</code></td>
