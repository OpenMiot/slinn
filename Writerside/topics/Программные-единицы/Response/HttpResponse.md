...
Объект класса является сборщиком HTTP-ответа из объектного представления

Наследуется от `slinn.HttpResponseHeader` и `slinn.HttpResponseChunk`

```Python
class slinn.HttpResponse (
    payload: typing.Any,
...
- `set_cookie` наследуется от `slinn.HttpResponseHeader`
- `make` наследуется от `slinn.HttpResponseHeader`
